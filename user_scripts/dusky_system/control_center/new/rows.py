...
Thread-safe and optimized for GTK3 on Arch Linux (Hyprland).
"""
...
class WidgetState:
    ...
    icon: PollSlot = field(default_factory=PollSlot)
    monitor: PollSlot = field(default_factory=PollSlot)
    value: PollSlot = field(default_factory=PollSlot)
    misc: PollSlot = field(default_factory=PollSlot)
...
def _perform_redirect(
    ...
    if not isinstance(child, Gtk.Stack):
        return
    ...
    for step_title in path_suffix:
        ...
        # Check if page is already pushed/exists in the Stack
        page = child.get_child_by_name(tag)
        ...
        if not page:
            ...
            # Give the child a name (equivalent to tag) so get_child_by_name works
            child.add_named(page, tag)

        page.show_all()
        child.set_visible_child_name(tag)
...
class DynamicIconMixin(AsyncPollingMixin):
    ...
    def _apply_icon_update(self, new_icon: str) -> None:
        ...
        if new_icon:
            res = self.icon_widget.get_icon_name()
            current_icon = res[0] if isinstance(res, tuple) else res
            if current_icon != new_icon:
                self.icon_widget.set_from_icon_name(new_icon, Gtk.IconSize.BUTTON)
...
# BASE ROW CLASS
# =============================================================================
class BaseActionRow(DynamicIconMixin, Gtk.ListBoxRow):
    ...
    def __init__(
        ...
        self.toast_overlay: Gtk.Overlay | None = self.context.get("toast_overlay")
        self.nav_view: Gtk.Stack | None = self.context.get("nav_view")
        ...
        self.main_box.pack_start(self.prefix_box, False, False, 0)

        # Labels Box
        self.text_vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        self.text_vbox.set_valign(Gtk.Align.CENTER)
        self.main_box.pack_start(self.text_vbox, True, True, 0)

        self.title_label = Gtk.Label(xalign=0.0)
        self.title_label.get_style_context().add_class("title")
        self.text_vbox.pack_start(self.title_label, False, False, 0)

        self.subtitle_label = Gtk.Label(xalign=0.0)
        self.subtitle_label.get_style_context().add_class("subtitle")
        self.subtitle_label.set_no_show_all(True)
        self.text_vbox.pack_start(self.subtitle_label, False, False, 0)

        # Suffix
        self.suffix_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self.suffix_box.set_valign(Gtk.Align.CENTER)
        self.main_box.pack_end(self.suffix_box, False, False, 0)

        title = str(properties.get("title", "Unnamed"))
        self.set_title(GLib.markup_escape_text(title, -1))
        if sub := properties.get("description", ""):
            self.set_subtitle(GLib.markup_escape_text(str(sub), -1))
    ...
    def set_subtitle(self, subtitle: str) -> None:
        if subtitle:
            self.subtitle_label.set_markup(subtitle)
            self.subtitle_label.show()
        else:
            self.subtitle_label.hide()

    def add_prefix(self, widget: Gtk.Widget) -> None:
        self.prefix_box.pack_start(widget, False, False, 0)
        widget.show()

    def add_suffix(self, widget: Gtk.Widget) -> None:
        self.suffix_box.pack_start(widget, False, False, 0)
        widget.show()

    def set_activatable_widget(self, widget: Gtk.Widget) -> None:
        pass
        ...
        if isinstance(icon, dict) and icon.get("type") == "file":
            if path := icon.get("path"):
                ...
                if p.exists():
                    ...
                    img.get_style_context().add_class("action-row-prefix-icon")
    ...
    def do_unrealize(self) -> None:
        ...
        Gtk.ListBoxRow.do_unrealize(self)
...
class ButtonRow(BaseActionRow):
    ...
    def __init__(
        ...
        if multi_buttons and isinstance(multi_buttons, list):
            ...
            self.btn.get_style_context().add_class("run-btn")
    ...
    def _apply_base_style(self, style: str) -> None:
        for s in ["suggested-action", "destructive-action", "default-action"]:
            self.btn.get_style_context().remove_class(s)
        match style:
            case "destructive":
            ...
            case _:
                self.btn.get_style_context().add_class("default-action")
...
class LabelRow(BaseActionRow):
    ...
    def __init__(
        ...
        self.value_label.get_style_context().add_class("dim-label")
    ...
    def _update_label(self, text: str) -> bool:
        ...
        if self.value_label.get_label() != text:
            ...
            self.value_label.get_style_context().remove_class("dim-label")
        return GLib.SOURCE_REMOVE
...
class SelectionRow(BaseActionRow):
    ...
    def __init__(
        ...
        if isinstance(raw_options, list) and raw_options:
            ...
            self._update_options_combobox(self.options_list)
    ...
    def _update_options_combobox(self, new_options: list[str]) -> None:
    ...
    def _on_selected(self, _combo: Gtk.ComboBoxText) -> None:
        if self._programmatic_update:
            return
        idx = self.get_selected()
        if idx < 0 or idx >= len(self.options_list):
            return
        item = self.options_list[idx]
...
class EntryRow(BaseActionRow):
    ...
    def __init__(
        ...
        # Embedded Entry in GTK3 suffix
        self.entry = Gtk.Entry()
        self.entry.set_valign(Gtk.Align.CENTER)
        self.entry.set_placeholder_text(str(properties.get("placeholder", "")))
    ...
    def set_show_apply_button(self, visible: bool) -> None:
        pass
...
class NavigationRow(BaseActionRow):
    ...
    def __init__(
        ...
        self.add_suffix(Gtk.Image.new_from_icon_name("go-next-symbolic", Gtk.IconSize.BUTTON))
        self.set_activatable(True)
        self.connect("activated", self._on_activated)

    def _on_activated(self, _row: Gtk.ListBoxRow) -> None:
        if self.nav_view and self.builder_func:
            ...
            tag = make_nav_tag(new_path)
            # Create subpage in history stack if it doesn't already exist
            page = self.nav_view.get_child_by_name(tag)
            if not page:
                page = self.builder_func(title, self.layout_data, new_ctx)
                self.nav_view.add_named(page, tag)
            page.show_all()
            self.nav_view.set_visible_child_name(tag)

class ExpanderRow(DynamicIconMixin, Gtk.ListBoxRow):
    ...
    def __init__(
        ...
        super().__init__()
        self.add_css_class("expander-row")
        ...
        self.prefix_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self.header_box.pack_start(self.prefix_box, False, False, 0)

        self.text_vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        self.text_vbox.set_valign(Gtk.Align.CENTER)
        self.header_box.pack_start(self.text_vbox, True, True, 0)
        ...
        self.child_listbox.get_style_context().add_class("nested-listbox")
        self.child_listbox.connect("row-activated", lambda lb, r: r.emit("activated") if hasattr(r, "emit") else None)
        self.expander.add(self.child_listbox)
    ...
    def add_row(self, row: Gtk.ListBoxRow) -> None:
        self.child_listbox.add(row)
        row.show()

    def _build_single_row(self, item: dict[str, object]) -> Gtk.ListBoxRow | None:
...
class AsyncSelectorRow(DynamicIconMixin, Gtk.ListBoxRow):
    ...
    def __init__(
        ...
        title_label.set_markup(f"<b>{GLib.markup_escape_text(title_str, -1)}</b>")
        ...
        self.refresh_btn.get_style_context().add_class("flat")
        ...
        self.dropdown.connect("changed", self._on_dropdown_selected)
        ...
        if self.has_action and not self.auto_execute:
            ...
            if button_style == "destructive":
                ...
                self.action_btn.get_style_context().add_class("default-action")
    ...
    def _on_dropdown_selected(self, _dropdown: Gtk.ComboBoxText) -> None:
        if self._programmatic_update or not self.has_action or not self.auto_execute:
            return
        selected_idx = self.dropdown.get_active()
        if not self.json_data or selected_idx < 0 or selected_idx >= len(self.json_data):
            return
        self._on_action_clicked()

    def _on_action_clicked(self, _btn: Gtk.Button | None = None) -> None:
        selected_idx = self.dropdown.get_active()
        if selected_idx < 0 or selected_idx >= len(self.json_data):
            return
...
class GridCardBase(Gtk.Button):
    ...
    def _build_content(self, icon: str, title: str) -> Gtk.Box:
        ...
        img.get_style_context().add_class("hero-icon")
        img.set_pixel_size(24)
        self.icon_widget = img

        self.title_label = Gtk.Label(label=title)
        self.title_label.get_style_context().add_class("hero-title")
...
class GridCard(DynamicIconMixin, GridCardBase):
    ...
    def __init__(
        ...
        if badge_path:
            ...
            self.badge_label.get_style_context().add_class("badge-label")
...
class GridToggleCard(DynamicIconMixin, StateMonitorMixin, GridCardBase):
    ...
    def __init__(
        ...
        self.status_lbl = Gtk.Label(label=STATE_OFF)
        self.status_lbl.get_style_context().add_class("hero-subtitle")
