...
Thread-safe utility library for GTK3 control center on Arch Linux (Hyprland).
...
def preflight_check() -> None:
    """
    Check for critical dependencies (GTK3, UWSM).
    ...
    try:
        ...
        missing_deps.append("python-gobject (GTK3)")
...
# =============================================================================
# UI HELPERS — GTK3 Toast System
# =============================================================================
_active_toast_source: int | None = None


def toast(
    toast_overlay: Gtk.Overlay | None, message: str, timeout: int = 2
) -> None:
    """Schedule a toast notification on the main thread using a GTK3 overlay."""
    if toast_overlay is None:
        return

    from gi.repository import GLib, Gtk as GtkLib

    def _show() -> bool:
        global _active_toast_source

        # Remove any existing toast label from the overlay
        for child in toast_overlay.get_children():
            if hasattr(child, "_is_toast_label") and child._is_toast_label:
                toast_overlay.remove(child)

        # Cancel any pending removal timer
        if _active_toast_source is not None:
            GLib.source_remove(_active_toast_source)
            _active_toast_source = None

        label = GtkLib.Label(label=message)
        label._is_toast_label = True
        label.get_style_context().add_class("toast-notification")
        label.set_halign(GtkLib.Align.CENTER)
        label.set_valign(GtkLib.Align.END)
        label.set_margin_bottom(16)
        label.show()
        toast_overlay.add_overlay(label)

        def _remove_toast() -> bool:
            global _active_toast_source
            _active_toast_source = None
            try:
                toast_overlay.remove(label)
            except Exception:
                pass
            return GLib.SOURCE_REMOVE

        _active_toast_source = GLib.timeout_add_seconds(timeout, _remove_toast)
        return GLib.SOURCE_REMOVE
