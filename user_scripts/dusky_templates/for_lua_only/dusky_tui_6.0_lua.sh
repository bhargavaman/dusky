...
validate_item_config() {
    ...
    # ADDED COMMA (,) to allow multi-scope global arrays!
...
write_value_to_file() {
    ...
    # Multi-scope intercept for Global tab support
    if [[ $requested_scope == *","* ]]; then
        local s final_status=0
        local -a scopes=()
        IFS=',' read -r -a scopes <<< "$requested_scope"
        for s in "${scopes[@]}"; do
            write_value_to_file "$requested_key" "$new_val" "$s" || final_status=$?
        done
        return $final_status
    fi
...
load_active_values() {
    ...
    for item in "${_lav_items_ref[@]:-}"; do
        ...
        # Read from the first scope if it's a multi-scope global tab
        local fetch_scope=$norm_scope
        if [[ $fetch_scope == *","* ]]; then fetch_scope="${fetch_scope%%,*}"; fi
        cache_key="${norm_key}|${fetch_scope}"
