#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Dusky Mako Configuration Editor (Dynamic Architecture)
# Based on Dusky TUI Engine v5.6
# -----------------------------------------------------------------------------
...
declare CONFIG_FILE="${MAKO_CONFIG_FILE:-${XDG_CONFIG_HOME}/mako/config}"
declare -r APP_TITLE="Mako Notification Engine"
declare -r APP_VERSION="v2.1"
...
declare -ra TABS=("Global Style" "Global Layout" "Behavior" "Overrides" "Tools")

declare -a DISCOVERED_SECTIONS=()

# Pre-parse the config to discover dynamic criteria blocks like [app-name=OSD]
discover_sections() {
    local line scope
    ...
    while IFS= read -r line; do
        if [[ $line =~ ^\[(.*)\]$ ]]; then
            ...
            local found=0
            for s in "${DISCOVERED_SECTIONS[@]:-}"; do
                [[ $s == "$scope" ]] && { found=1; break; }
            done
            (( found )) || DISCOVERED_SECTIONS+=("$scope")
        fi
    done < "$CONFIG_FILE"
}
...
register_items() {
    # 0. Global Style
    register 0 "Font"                 'font|string||||'               "monospace 10"
    register 0 "Background Color"     'background-color|string||||'   ""
    register 0 "Text Color"           'text-color|string||||'         ""
    register 0 "Border Color"         'border-color|string||||'       ""
    register 0 "Progress Color"       'progress-color|string||||'     ""
    register 0 "Border Size (px)"     'border-size|int||0|50|1'       "2"
    register 0 "Border Radius"        'border-radius|string||||'      "10"
    register 0 "Show Icons"           'icons|cycle||0,1||'            "1"
    register 0 "Max Icon Size"        'max-icon-size|int||16|256|8'   "48"
    register 0 "Icon Radius"          'icon-border-radius|int||0|100|1' "0"
    register 0 "Icon Path"            'icon-path|string||||'          ""
    register 0 "Enable Markup"        'markup|cycle||0,1||'           "1"
    register 0 "Text Alignment"       'text-alignment|cycle||left,center,right||' "left"
    register 0 "Format String"        'format|string||||'             "<b>%s</b>\n%b"

    # 1. Global Layout
    register 1 "Width (px)"           'width|int||1|2000|10'          "300"
    register 1 "Height (px)"          'height|int||1|2000|10'         "100"
    register 1 "Margin (px)"          'margin|string||||'             "10"
    register 1 "Padding (px)"         'padding|string||||'            "10"
    register 1 "Outer Margin"         'outer-margin|string||||'       ""
    register 1 "Anchor Position"      'anchor|cycle||top-right,top-center,top-left,bottom-right,bottom-center,bottom-left,center,left,right||' "top-right"
    register 1 "Layer"                'layer|cycle||background,bottom,top,overlay||' "overlay"

    # 2. Behavior
    register 2 "Default Timeout (ms)" 'default-timeout|int||0|60000|500' "5000"
    register 2 "Ignore Timeout"       'ignore-timeout|cycle||0,1||'   "0"
    register 2 "Max Visible"          'max-visible|int||1|50|1'       "5"
    register 2 "Sort Order"           'sort|cycle||-time,+time,-priority,+priority||' "-time"
    register 2 "Save History"         'history|cycle||0,1||'          "1"
    register 2 "Max History"          'max-history|int||1|1000|5'     "50"
    register 2 "Enable Actions"       'actions|cycle||0,1||'          "1"
    register 2 "Global On-Notify"     'on-notify|string||||'          "none"
    register 2 "Left Click Action"    'on-button-left|string||||'     "invoke-default-action"
    register 2 "Right Click Action"   'on-button-right|string||||'    "exec makoctl menu -n \"\$MAKO_NOTIFICATION_ID\""

    # 3. Dynamic Overrides & Criteria
    register 3 "[+] Create New Override..." 'create_block|action||||' ""

    local i=0
    for scope in "${DISCOVERED_SECTIONS[@]:-}"; do
        local safe_id="menu_dyn_${i}"
        register 3 "[$scope]" "${safe_id}|menu||||" ""
        
        # Inject standard granular fields tailored to this specific block
        register_child "$safe_id" "Anchor Position"  "anchor|cycle|${scope}|top-right,top-center,top-left,bottom-right,bottom-center,bottom-left,center,left,right||" ""
        register_child "$safe_id" "Background Color" "background-color|string|${scope}|||" ""
        register_child "$safe_id" "Text Color"       "text-color|string|${scope}|||" ""
        register_child "$safe_id" "Border Color"     "border-color|string|${scope}|||" ""
        register_child "$safe_id" "Border Size"      "border-size|int|${scope}|0|50|1" ""
        register_child "$safe_id" "Border Radius"    "border-radius|string|${scope}|||" ""
        register_child "$safe_id" "Width"            "width|int|${scope}|1|2000|10" ""
        register_child "$safe_id" "Height"           "height|int|${scope}|1|2000|10" ""
        register_child "$safe_id" "Margin"           "margin|string|${scope}|||" ""
        register_child "$safe_id" "Padding"          "padding|string|${scope}|||" ""
        register_child "$safe_id" "Outer Margin"     "outer-margin|string|${scope}|||" ""
        register_child "$safe_id" "Default Timeout"  "default-timeout|int|${scope}|0|60000|500" ""
        register_child "$safe_id" "Format String"    "format|string|${scope}|||" ""
        register_child "$safe_id" "Max Icon Size"    "max-icon-size|int|${scope}|16|256|8" ""
        register_child "$safe_id" "Invisible (DND)"  "invisible|cycle|${scope}|0,1||" ""
        register_child "$safe_id" "On-Notify Exec"   "on-notify|string|${scope}|||" ""
        register_child "$safe_id" "Left Click Action" "on-button-left|string|${scope}|||" ""
        
        # Fix: Using += 1 instead of ++ to prevent evaluation to 0 and tripping set -e
        (( i += 1 ))
    done

    # 4. Tools
    register 4 "Reload Daemon"        'reload_mako|action||||'        ""
    register 4 "Test Notification"    'test_notify|action||||'        ""
    register 4 "Dismiss All Messages" 'dismiss_all|action||||'        ""
}

action_create_block() {
    local new_scope=""
    prompt_line_input "Criteria (e.g. app-name=OSD):" new_scope
    if [[ -n $new_scope ]]; then
        # Strip accidental brackets if user typed them
        new_scope=$(echo "$new_scope" | sed -e 's/^\[//' -e 's/\]$//')
        if [[ -n $new_scope ]]; then
            # Ensure file exists
            [[ -f $WRITE_TARGET ]] || touch "$WRITE_TARGET"
            echo -e "\n[$new_scope]" >> "$WRITE_TARGET"
            
            # Restart TUI to seamlessly parse and inject the new block into the menu
            cleanup
            exec "$0" "$@"
        fi
    fi
...
action_reload_mako() {
    if command -v makoctl >/dev/null 2>&1; then
        ...
        set_status "Mako configuration reloaded."
    else
        set_status "Error: 'makoctl' command not found."
    fi
}

action_test_notify() {
    if command -v notify-send >/dev/null 2>&1; then
        notify-send -h string:x-dunst-stack-tag:mako-preview \
                    -u normal \
                    -t 4000 \
                    -i preferences-desktop-notification \
                    "Dusky TUI Simulator" \
                    "This is a live preview test notification." >/dev/null 2>&1 || :
        set_status "Test notification sent."
    else
        set_status "Error: 'notify-send' command not found."
    fi
}
...
action_dismiss_all() {
    if command -v makoctl >/dev/null 2>&1; then
        ...
        set_status "All notifications dismissed."
    else
        set_status "Error: 'makoctl' command not found."
    fi
}
...
do_live_preview() {
    ...
    if command -v notify-send >/dev/null 2>&1; then
        notify-send -h string:x-dunst-stack-tag:mako-preview \
                    ...
                    -i preferences-desktop-notification \
                    "Dusky Mako TUI" \
                    "Live Configuration Preview" >/dev/null 2>&1 || :
    fi
}
...
cleanup() {
    ...
    # Clean up the flock file from /tmp on exit
    [[ -n ${LOCK_TARGET:-} && -f $LOCK_TARGET ]] && rm -f -- "$LOCK_TARGET" 2>/dev/null || :
...
resolve_write_target() {
    ...
    # Safely route the lock file to /tmp
...
validate_item_config() {
    ...
    # Patched Regex: Allows quotes, spaces, tildes, etc. for Mako Criteria Blocks
    local re='^[a-zA-Z0-9_.: =/"~'\''-]+$'
...
render_item_list() {
    ...
    for (( ri = vs; ri < ve; ri++ )); do
        ...
        case $type in
            menu) display="${C_YELLOW}[+] Open Submenu...${C_RESET}" ;;
...
draw_detail_view() {
    ...
    buf+="${CURSOR_HOME}${C_MAGENTA}┌${H_LINE}┐${C_RESET}${CLR_EOL}"$'\n'
    title=" CRITERIA BLOCK "; sub=" ${CURRENT_MENU_ID} "
...
main() {
    ...
    # Pre-parse the config file to discover dynamic blocks
    discover_sections
    ...
    while true; do
        ...
        if IFS= read -rsn1 -t "$READ_LOOP_TIMEOUT" key < /dev/tty; then
            ...
            # If the loop times out (user stopped spamming keys for 250ms), execute preview
