#!/usr/bin/env python3
"""
040_disk_mount.py - DUSKY Final - Python 3.14.6 + Rich 15.0.0
Fixes:
...
def make_console():
    ...
    if term in ("dumb","unknown"): return Console(color_system=None,force_terminal=False,no_color=True,legacy_windows=False)
...
DUSKY_EFI_LABEL="DUSKY_EFI"; DUSKY_ROOT_LABEL="DUSKY_ROOT"
...
def run(*cmd,check=True,capture=True,input_text=None,timeout=300):
    ...
    try:
        if isinstance(input_text,(bytes,bytearray)): return subprocess.run(argv,check=check,text=False,capture_output=capture,input=bytes(input_text),timeout=timeout)
        elif isinstance(input_text,str): return subprocess.run(argv,check=check,text=True,capture_output=capture,input=input_text,timeout=timeout)
...
def unmount_mount_tree():
    ...
    for fs in mnts:
        t=fs.get("target","")
        if t=="/mnt" or t.startswith("/mnt/"): targets.append(t)
...
def is_empty_dir(p:Path):
    try:
        ...
        with os.scandir(p) as it: return next(it,None) is None
    except: return False
def ensure_subvolume(path:Path,nocow=False):
    if path.exists():
        r=run("btrfs","subvolume","show",str(path),check=False,capture=True)
        if r.returncode!=0:
            console.print(f"[red]{path} exists not subvol[/red]"); sys.exit(1)
        existed=True
    else:
...
def load_state():
    ...
    if STATE_JSON.exists():
        try: state.update(json.loads(STATE_JSON.read_text()))
        except: pass
    if STATE_ENV.exists():
        try:
            ...
            for line in r.stdout.splitlines():
                ...
                k,v=line.split("=",1)
                if not v: continue
                if k=="PROVISIONED_ROOT_PART": state.setdefault("root_part",v)
                elif k=="PROVISIONED_EFI_PART": state.setdefault("efi_part",v)
                elif k=="ENCRYPT_ROOT": state.setdefault("encrypt",v=="1")
        except: pass
    return state
def get_partition_path(disk,num):
    ...
    name=Path(disk).name
    if re.search(r"(?:nvme\d+n\d+|mmcblk\d+|loop\d+|nbd\d+|pmem\d+)$",name) or (disk and disk[-1].isdigit()): return f"{disk}p{num}"
    return f"{disk}{num}"
def determine_root_partition(auto_mode):
    ...
    use_crypt=False
    if isinstance(encrypt_hint,bool): use_crypt=encrypt_hint
    elif has_mapper: use_crypt=True
    if use_crypt:
        mapped=Path("/dev/mapper/cryptroot")
        if not mapped.exists(): console.print("[red]LUKS expected no mapper[/red]"); sys.exit(1)
        backing=""
        try:
            for dm in Path("/sys/class/block").iterdir():
                if not dm.name.startswith("dm-"): continue
                try:
                    if (dm/"dm"/"name").read_text().strip()=="cryptroot":
                        slaves=list((dm/"slaves").iterdir())
                        if slaves: backing=f"/dev/{slaves[0].name}"; break
                except: continue
        except: pass
        if not backing:
            r=run("cryptsetup","status","cryptroot",check=False,capture=True)
            for line in r.stdout.splitlines():
                if line.strip().lower().startswith("device:"): backing=line.split(":",1)[1].strip(); break
        if not backing: console.print("[red]No backing[/red]"); sys.exit(1)
        root_part=Path(backing).resolve(); mapped_root=mapped
    else:
        if auto_mode:
            ...
            if prov and Path(prov).exists():
                ...
                for line in r.stdout.splitlines():
                    ...
                    name=cols[0]; fstype=cols[1]; label=cols[2] if len(cols)>2 else ""
                    if fstype=="btrfs":
                        if label==DUSKY_ROOT_LABEL: duskies.append(name)
                        btrfs_parts.append(name)
                if len(duskies)==1: root_part=Path(duskies[0]).resolve(); mapped_root=root_part
                elif len(btrfs_parts)==1: root_part=Path(btrfs_parts[0]).resolve(); mapped_root=root_part
                else: console.print("[red]Cannot auto-detect btrfs root[/red]"); sys.exit(1)
        else:
            r=run("lsblk","-l","-o","NAME,SIZE,TYPE,FSTYPE,LABEL,PARTLABEL",check=False,capture=True); console.print(r.stdout)
            while True:
                raw=Prompt.ask("Enter DUSKY BTRFS root (e.g. vda2)",console=console)
                if not VALID_PART_RE.match(raw): console.print("[red]Invalid[/red]"); continue
                name=raw.removeprefix("/dev/")
                p=Path("/dev")/name
                try:
                    rp=p.resolve()
                    if not rp.exists(): console.print(f"[red]{rp} no exist[/red]"); continue
                    root_part=rp; mapped_root=rp; break
                except Exception as e: console.print(f"[red]{e}[/red]")
    if not root_part.exists(): console.print(f"[red]{root_part} invalid[/red]"); sys.exit(1)
    try:
        ...
        if pk: root_disk=Path(f"/dev/{pk}").resolve()
        else: raise ValueError
    except:
        import re as _re
        m=_re.match(r"^(.*?)(?:p?\d+)$",root_part.name)
        if m: root_disk=Path(f"/dev/{m.group(1)}").resolve()
        else: console.print(f"[red]Failed parent disk[/red]"); sys.exit(1)
    return mapped_root, root_part, root_disk
def validate_root_state(mapped_root):
    if not mapped_root.exists(): console.print(f"[red]{mapped_root} not found[/red]"); sys.exit(1)
    r=run("lsblk","-ndlo","FSTYPE",str(mapped_root),check=False,capture=True)
    if r.stdout.strip()!="btrfs": console.print(f"[red]{mapped_root} not btrfs[/red]"); sys.exit(1)
def validate_efi_partition(part):
    ...
    if EFI_GPT_TYPE not in out and "vfat" not in out and "fat32" not in out:
        console.print(f"[red]{part} not ESP[/red]"); sys.exit(1)
def is_mounted(dev):
    try:
...
def auto_detect_efi_partition(root_disk,root_part):
    try:
        ...
        data=json.loads(r.stdout); bdevs=data.get("blockdevices",[])
        if not bdevs: return None
        children=bdevs[0].get("children",[]) if bdevs[0].get("type")=="disk" else bdevs
        guid=[]; dusky=[]; labelm=[]; vfat=[]; non_root=[]
        for ch in children:
            ptype=(ch.get("parttype") or "").lower(); fstype=(ch.get("fstype") or "").lower()
            partlabel=ch.get("partlabel") or ""; label=ch.get("label") or ""; name=ch.get("path") or ch.get("name")
            if not name: continue
            pp=Path(name).resolve()
            if pp==root_part.resolve(): continue
            if ch.get("type")!="part": continue
            non_root.append(pp)
            if ptype==EFI_GPT_TYPE:
                guid.append(pp)
                if label==DUSKY_EFI_LABEL or partlabel==DUSKY_EFI_LABEL: dusky.append(pp)
            if "efi" in partlabel.lower(): labelm.append(pp)
            if fstype in ("vfat","fat32"): vfat.append(pp)
        if len(dusky)==1: return dusky[0]
        if len(guid)==1: return guid[0]
        if len(labelm)==1: return labelm[0]
        if len(vfat)==1: return vfat[0]
        if len(non_root)==1: return non_root[0]
    except: pass
    return None
def prompt_for_efi_partition(root_disk):
    r=run("lsblk","-l","-o","NAME,SIZE,TYPE,FSTYPE,PARTTYPE,PARTLABEL,LABEL",str(root_disk),check=False,capture=True); console.print(r.stdout)
    while True:
...
def determine_efi_partition(auto_mode,root_disk,root_part):
    if BOOT_MODE!="UEFI": return None
    state=load_state()
    if auto_mode:
        prov=state.get("efi_part")
        if prov and Path(prov).exists():
            console.print(f"[cyan]Auto EFI {prov}[/cyan]"); return Path(prov).resolve()
        det=auto_detect_efi_partition(root_disk,root_part)
        if det: console.print(f"[cyan]Auto EFI {det}[/cyan]"); return det
...
def construct_subvolume_matrix(mapped_root):
    ...
    try:
        ...
        for sub in STD_SUBVOLS: ensure_subvolume(tmpdir/sub,nocow=False)
        for sub in NOCOW_SUBVOLS: ensure_subvolume(tmpdir/sub,nocow=True)
...
def assemble_fhs(mapped_root,efi_part):
    ...
    for mp in ["home",".snapshots","var/log","var/cache","var/tmp","var/lib/machines","var/lib/portables","var/lib/libvirt","var/lib/mysql","var/lib/postgres","swap","boot","home/.snapshots"]:
...
def initialize_swapfile():
    ...
    if SWAPFILE_PATH.exists() and not SWAPFILE_PATH.is_file():
        console.print(f"[red]{SWAPFILE_PATH} not regular file[/red]"); sys.exit(1)
        ...
        try:
            if SWAPFILE_PATH.stat().st_size==4*1024**3:
                if run("swapon",str(SWAPFILE_PATH),check=False,capture=True).returncode==0:
                    console.print("[green]>> Swap re-activated[/green]"); return
        except: pass
        try: SWAPFILE_PATH.unlink()
        except: pass
...
def run_common(auto_mode):
    ...
    if BOOT_MODE=="UEFI":
        ...
        if efi_part:
            efi_part=efi_part.resolve(); validate_efi_partition(efi_part)
            tmp_obj=None
            try:
                mnt=is_mounted(str(efi_part)); tp=mnt
                if not mnt:
                ...
                if tp and Path(tp,"EFI","Microsoft").is_dir():
                    console.print(f"[cyan]Dual-boot Windows on {efi_part}, preserving[/cyan]")
                if tmp_obj: run("umount",tp,check=False,capture=True)
            except:
                try:
                    if tmp_obj: run("umount",tp,check=False,capture=True)
                except: pass
                ...
                try:
                    if tmp_obj: tmp_obj.cleanup()
                except: pass
    construct_subvolume_matrix(mapped_root)
    assemble_fhs(mapped_root,efi_part)
    initialize_swapfile()
    console.print("\n[bold green]>> DUSKY Setup Complete[/bold green]")
    try:
        r=run("lsblk","-l","-f",str(root_disk),check=False,capture=True); console.print(r.stdout)
    except: pass
    try:
        r=run("findmnt","-R","/mnt",check=False,capture=True); console.print(r.stdout)
    except: pass
def run_auto_mode():
    console.print(Panel(f"[bold]AUTONOMOUS DUSKY BTRFS MOUNT ({BOOT_MODE})[/bold]",box=box.ROUNDED))
    run_common(True)
def run_interactive_mode():
    console.print(Panel(f"[bold]INTERACTIVE DUSKY BTRFS MOUNT ({BOOT_MODE})[/bold]",box=box.ROUNDED))
    run_common(False)
def main():
    parser=argparse.ArgumentParser(description="DUSKY 040")
    ...
    def _h(sig,frame):
        console.print(f"\n[yellow]Signal {signal.Signals(sig).name}[/yellow]"); teardown_state(); sys.exit(128+sig)
    signal.signal(signal.SIGINT,_h); signal.signal(signal.SIGTERM,_h)
    try:
        if args.auto: run_auto_mode()
        else:
            if Confirm.ask("Run AUTONOMOUS?",console=console,default=True): run_auto_mode()
            else: run_interactive_mode()
    except KeyboardInterrupt:
        console.print("[red]Interrupted[/red]"); teardown_state(); sys.exit(130)
    except Exception as e:
        console.print(f"[red]Fatal {e}[/red]"); import traceback; traceback.print_exc(); teardown_state(); sys.exit(1)
